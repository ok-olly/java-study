...
public class World {
	...
	public void randomize() {
		...
		for(int i=0; i<(rows*columns)/10; i++) {//횟수는 줄과 열이 많으면 많아진다.
			int row = random.nextInt(rows); //랜덤으로 0부터 rows(전체 줄 수)-1까지 랜덤생성
			int col = random.nextInt(columns); //0부터 ~ columns-1
